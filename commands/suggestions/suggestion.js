...
module.exports = {
    ...
    options: [
        ...
        {
            name: 'reason',
            description: 'The reason for the action. put "none" if you don\'t want one',
            required: true,
            type: 'STRING'
        }
    ],
    ...
    callback: async ({ message, interaction, args, guild }) => {
        ...
        if (!newStatus) {
            message.channel.send(`Unknown status "${status}", please use ${Object.keys(statusMessages)}`)
            message.delete()
            return
        }
        ...
        if (oldEmbed.fields.length === 2) {
            embed.addFields(oldEmbed.fields[0], {
                name: 'Status',
                value: `${newStatus.text}${reason.toLowerCase() == 'none' ? '': ` Reason: ${reason}`}`
            })
        } else {
            embed.addFields({
                name: 'Status',
